...
/* ── landing ─────────────────────────────────────────────────────────────────
   ...
   held apart from the workspace canvas, so the threshold reads as its own
   surface without going dark. */
body.phase-landing{background:#FBFCFE}
...
#view-landing{min-height:100vh;display:grid;grid-template-columns:minmax(0,1fr);
  align-content:center;padding:clamp(32px,5vh,72px) clamp(20px,6vw,80px);background:#FBFCFE}
...
# ── option helpers reused by the select-based pickers ────────────────────────
OPT_JS = r"""
