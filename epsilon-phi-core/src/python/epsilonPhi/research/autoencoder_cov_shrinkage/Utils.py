...
def ShowCovarsWeights(covmats, weights, names, fileName=None):
    ...
    axarr[0, 0].imshow(covmats[0], interpolation='none', aspect=1, vmin=1.0 * np.min(-0.0005, np.min(covmats[0])),
                       vmax=1.0 * np.max(0.0005, np.max(covmats[0])), cmap='seismic')
    axarr[1, 0].set_title('AutoEncoder Covariance')
    axarr[1, 0].imshow(covmats[1], interpolation='none', aspect=1, vmin=1.0 * np.min(-0.0005, np.min(covmats[1])),
                       vmax=1.0 * np.max(0.0005, np.max(covmats[1])), cmap='seismic')
