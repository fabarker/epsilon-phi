...
if __name__ == "__main__":
    ...
    df_dict = pd.read_excel('/Users/francisbarker/Desktop/Frequencies.xlsx', sheet_name=None)
    df_ = pd.concat([df_dict.get(x) for x in df_dict.keys()], axis=0)

    not_B = df_[df_.get('freq') != 'B'].set_index('uid')
    uids = not_B.index.to_list()
    q = session.query(TimeSeriesSpec).filter(TimeSeriesSpec.uid.in_(uids)).all()

    for spec in q:
        spec.frequency = not_B.loc[spec.uid].get('freq')
    ...
    specs = session.query(ImpliedVolatility).all()
    ...
    for spec in specs:
        print(len(specs) - ctr)
        ctr += 1
        q = session.query(Equity).filter(Equity.uid == spec.uid)
        res = sessionMgr.query_format_df(q).set_index('date')
        try:
            freq.extend([(spec.uid, spec.ticker, res.index.inferred_freq)])
        except:
            freq.extend([(spec.uid, spec.ticker, np.nan)])
    df_ = pd.DataFrame(freq, columns=['uid', 'ticker', 'freq'])
    df_.to_clipboard()
