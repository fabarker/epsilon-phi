...
class FFDataReader(object):
    ...
    def get_FFS1B1(frequency=Frequency.DAILY):
        ...
        return dataset.get('SMALL LoBM').to_frame('FFS1B1') / 100
    ...
    def get_FFS1B2(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        return dataset.get('ME1 BM2').to_frame('FFS1B2') / 100

    @staticmethod
    def get_FFS1B3(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        return dataset.get('SMALL HiBM').to_frame('FFS1B3') / 100

    @staticmethod
    def get_FFS2B1(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        return dataset.get('BIG LoBM').to_frame('FFS2B1') / 100

    @staticmethod
    def get_FFS2B2(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        return dataset.get('ME2 BM2').to_frame('FFS2B2') / 100

    @staticmethod
    def get_FFS2B3(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        return dataset.get('BIG HiBM').to_frame('FFS2B3') / 100

    @staticmethod
    def get_double_sorts_size_value(frequency=Frequency.DAILY):
        dataset = FFDataReader.get_dataset('6_Portfolios_2x3' + FFDataReader.freq_str(frequency))
        dataset.columns = ['FFS1B1', 'FFS1B2', 'FFS1B3', 'FFS2B1', 'FFS2B2', 'FFS2B3']
        return dataset / 100
