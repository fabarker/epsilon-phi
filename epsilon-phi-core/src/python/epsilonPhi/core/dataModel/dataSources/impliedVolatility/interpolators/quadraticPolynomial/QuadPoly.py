...
class QuadPoly(object):
    ...
    def set_ivols(self, ivols):
        _ivols = ivols.reset_index(drop=True)
    ...
    def interpolate(self, dates=None, maturities=None, strikes=None):
        ...
        t_vec = self.t.astype(float)
        x_vec = self.x.astype(float)
        v_vec = self.sig.astype(float)

        interp = np.full(shape=(NX*NM, ND), fill_value=np.nan)
        for t in tqdm(range(ND), desc="Interpolating Vol Surface"):
            idx = (unique_d[t] == self.dates)
            T = ND - 1
        ...
        _ivols.columns = pd.MultiIndex.from_tuples(list(zip(['vol'] * NM, _ivols.columns)))
        return _ivols.copy()
