...
cvar_ptf = ptf.deepcopy(name="CVaR Constrained")
...
def f_rtn(v):
    ...
    cvar, _ = cvar_ptf.get_portfolio_var_pol().get_VaR(11)
    return cvar - tgt_cvar

v_rtn = root_scalar(f_rtn, bracket=[0.1, 0.102], method="brentq").root
f_rtn(v_rtn)


# copy the current portfolio
cvar_ptf_2 = ptf.deepcopy(name="CVaR Constrained (No Private Assets)")
...
def f_rtn_2(v):
    ...
    cvar, _ = cvar_ptf_2.get_portfolio_var_pol().get_VaR(11)
    return cvar - tgt_cvar

v_rtn_2 = root_scalar(f_rtn_2, bracket=[0.09, 0.106], method="brentq").root
f_rtn_2(v_rtn_2)

ptfs = [ptf, cvar_ptf, cvar_ptf_2]
report = Reporting(
    os.getcwd(),
    "CVaR Matched"
)
